...
function createMoviesDivAll(moviesObj) {
  let divEl = document.createElement('div');
  divEl.className = 'movie';
  divEl.innerHTML = `<img src="img/${moviesObj.poster}" alt="${moviesObj.title}" class="movie__poster">
    <div class="movie__info">
      <h2 class="movie__title">${moviesObj.title}</h2>
      <h3 class="movie__genre">${moviesObj.genres.join(', ')}</h3>
    </div>`;
  return divEl;
};

function createIndexHtml(indexEl) {
  let indexElem = createMoviesDivAll(indexEl);
  let addBtn = document.createElement('button');
  addBtn.className = 'movie__btn add-btn';
  addBtn.setAttribute('data-id', indexEl.id);
  addBtn.innerText = 'Add to watchlist';
  indexElem.appendChild(addBtn);
  return indexElem
};

function createWatchlistHtml(watchEl) {
  let watchElem = createMoviesDivAll(watchEl);
  let removeBtn = document.createElement('button');
  removeBtn.className = 'movie__btn delete-btn';
  removeBtn.setAttribute('data-id', watchEl.id);
  removeBtn.innerText = 'Remove from watchlist';
  watchElem.appendChild(removeBtn);
  return watchElem;
};
...
if (moviesEl) {
  function appendMovies() {
    movies.forEach(el => {
      let movieDiv = createIndexHtml(el);
      moviesEl.appendChild(movieDiv);
    });
  };
};

if (moviesEl) appendMovies();

function filtermoviesByGenres(genre) {
...
if (sidebarEl) {
  sidebarEl.addEventListener('click', function (event) {
    if (event.target.className == 'genre__item') {
      let selectedGanre = event.target.innerText;
      let selectedMovies;
      if (selectedGanre !== 'All') {
        selectedMovies = filtermoviesByGenres(selectedGanre);
      } else {
        selectedMovies = movies;
      }
      moviesEl.innerHTML = '';
      appendSelectedMovies(selectedMovies);
    };
  });
};

function appendSelectedMovies(selektMovies) {
  selektMovies.forEach(el => {
    let movieDiv = createIndexHtml(el);
    moviesEl.appendChild(movieDiv);
  });
};
...
function searchMovie(searchMovie) {
  let resultArr = movies.filter(el => {
    return el.title.toLowerCase().includes(searchMovie);
  });
  return resultArr;
};
...
if (moviesEl) {
  searchEl.addEventListener('keyup', function () {
    let searchedMovies = searchMovie(searchEl.value);
    moviesEl.innerHTML = '';
    appendSelectedMovies(searchedMovies);
  });
};

let myWatchList = watchItemsFromLocalStorage();

function addmovieToWatch(movieId) {
...
if (moviesEl) {
  moviesEl.addEventListener('click', function (event) {
    ...
    let idOfCurrentMovie = +event.target.getAttribute('data-id');
    addmovieToWatch(idOfCurrentMovie);
    watchItemsToLocalStorage();
  });
};

function watchItemsToLocalStorage() {
  localStorage.setItem('myWatch', JSON.stringify(myWatchList));
};

function watchItemsFromLocalStorage() {
  let result = JSON.parse(localStorage.getItem('myWatch'));
  return result ? result : [];
};

if (watchEl) showWatchItems();


function showWatchItems() {
  watchEl.innerHTML = '';
  let myWatchListArr = watchItemsFromLocalStorage();
  myWatchListArr.forEach(el => {
    let movie = movies.filter((item) => {
      return item.id == el;
    });
    let element = createWatchlistHtml(movie[0]);
    watchEl.appendChild(element);
  });
};

if (watchEl) {
  watchEl.addEventListener('click', function (event) {
    if (event.target.className.includes('delete-btn')) {
      let elId = event.target.getAttribute('data-id');
      let myWatchListArr = watchItemsFromLocalStorage();
      myWatchListArr.splice(myWatchListArr.indexOf(+elId), 1);
      localStorage.setItem('myWatch', JSON.stringify(myWatchListArr));

      let parentTr = event.target.closest('.movie');
